    ...
    <>
      ...
      <div className="mb-4 flex flex-col flex-wrap md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex flex-col md:flex-row">
          <Button className="ml-0 md:ml-4 mt-4 md:mt-0" tag={Link} to="/app/hizmet_ve_urunler/ekle">Hizmet / Ürün Ekle</Button>
        </div>
      </div>
            ...
            {data.products.edges.map(({cursor, node}) => (
              <TableRow key={node.id}>
                <TableCell>
                  ...
                  />
                    <div>
                      <Link className="ml-3 font-semibold" to={`/app/hizmet_ve_urunler/${node.id}/detay`}>
                        {node.name}
                      </Link>
                    </div>
                  </div>
